...
public class Utils {
    ...
    public static List<Customer> getCustomers(){
        if(customers == null){
            ...
            customers.add(new Customer(1, "Hoang Phong", R.drawable.avatar_small));
            customers.add(new Customer(2, "Kim Phung", R.drawable.avatar_small));
            customers.add(new Customer(3, "Chi Huy", R.drawable.avatar_small));
            customers.add(new Customer(4, "Phuong Thao", R.drawable.avatar_small));
            customers.add(new Customer(5, "Anh Quoc", R.drawable.avatar_small));
            customers.add(new Customer(6, "Hung Dung", R.drawable.avatar_small));
            customers.add(new Customer(7, "Tung Chua", R.drawable.avatar_small));
        }
        return customers;
    }
    ...
    public static PhotoService getServiceById(long id) {
        Optional<PhotoService> first = getServices().stream().filter(photoService -> photoService.getId() == id).findFirst();
        return first.orElse(null);
    }

    public static Book getBooking(int id) {
        Optional<Book> first = getBooks().stream().filter(book -> book.getId() == id).findFirst();
        return first.orElse(null);
    }
